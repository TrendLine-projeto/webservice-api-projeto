...
exports.criarFornecedor = (fornecedor, callback) => {
    mysql.getConnection((error, conn) => {
        ...
        // Primeiro valida se o cliente existe
        ...
        conn.query(queryVerificaCliente, [fornecedor.cliente_id], (error, resultsCliente) => {
            ...
            if (resultsCliente.length === 0) {
                conn.release();
                return callback({ tipo: 'ClienteNaoEncontrado', mensagem: 'Cliente não encontrado. Verifique o ID informado.' });
            }

            // Se cliente existir, cria o fornecedor
...
exports.buscarFornecedoresPorCliente = (filtros, callback) => {
    ...
    const queryTotal = `
        SELECT COUNT(*) AS total FROM fornecedor_suprimentos
        ${where}
    `;
