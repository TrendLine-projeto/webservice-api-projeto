...
import { FiltrosMateriaPrima } from '../types/estoque/EstoqueMateriaPrima';

export const construirWhereMateriaPrima = (filtros: FiltrosMateriaPrima): { where: string; params: any[] } => {
...
export const estoquemateriaprimaPorId = async (id: number): Promise<{ materiaPrima: RowDataPacket[] }> => {
  const queryBusca = `
    SELECT * FROM estoque_materiaprima WHERE id = ?
  `;

  const conn: PoolConnection = await pool.getConnection();
  try {
    const [materiaPrima] = await conn.query(queryBusca, [id]);
    return {
      materiaPrima: materiaPrima as RowDataPacket[]
    };
  } catch (error) {
    throw error;
  } finally {
...
export const inserirMateriaPrima = async (materiaPrima: any): Promise<ResultSetHeader> => {
  const conn: PoolConnection = await pool.getConnection();
  try {
    const sql = `
      INSERT INTO estoque_materiaprima (
        nome,
        tipo,
        descricao,
        unidade,
    ...
    const values = [
      ...
      materiaPrima.largura,
      materiaPrima.comprimento,
      materiaPrima.gramatura,
      materiaPrima.cor,
      materiaPrima.marca,
      materiaPrima.localArmazenamento,
      materiaPrima.codigoBarras,
      materiaPrima.precoUnitario,
      materiaPrima.fornecedor,
      materiaPrima.dataUltimaEntrada,
      materiaPrima.observacoes,
      materiaPrima.idFornecedor_suprimentos,
      materiaPrima.idCliente
    ];
...
export const buscarTotal = async (filtros: FiltrosMateriaPrima): Promise<number> => {
  const conn = await pool.getConnection();
  try {
    const { where, params } = construirWhereMateriaPrima(filtros);
    const query = `SELECT COUNT(*) AS total FROM estoque_materiaprima emp JOIN fornecedor_suprimentos fs ON emp.idFornecedor_suprimentos = fs.id ${where}`;
...
export const buscarLista = async (filtros: FiltrosMateriaPrima): Promise<RowDataPacket[]> => {
  const conn = await pool.getConnection();
  try {
    const { where, params } = construirWhereMateriaPrima(filtros);
...
export const excluirPorId = async (id: number): Promise<ResultSetHeader> => {
  ...
  try {
    const [result] = await conn.query<ResultSetHeader>(
      "DELETE FROM estoque_materiaprima WHERE id = ?",
      [id]
    );
    return result;
  } finally {
...
export const atualizar = async (id: number, dados: any): Promise<boolean> => {
  ...
  try {
    const [result] = await conn.query<ResultSetHeader>(
      `UPDATE estoque_materiaprima SET ? WHERE id = ?`,
      [dados, id]
    );
